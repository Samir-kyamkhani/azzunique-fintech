...
ALTER TABLE `wallets` ADD CONSTRAINT `wallet_tenant_fk` FOREIGN KEY (`tenant_id`) REFERENCES `tenants`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX `idx_wallet_tenant_status` ON `wallets` (`tenant_id`,`status`);--> statement-breakpoint
