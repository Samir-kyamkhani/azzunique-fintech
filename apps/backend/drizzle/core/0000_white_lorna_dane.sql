...
CREATE TABLE `platform_service_providers` (
	...
	CONSTRAINT `uniq_platform_service_provider` UNIQUE(`platform_service_id`,`service_provider_id`)
);
--> statement-breakpoint
...
CREATE INDEX `idx_provider_active` ON `service_providers` (`is_active`);--> statement-breakpoint
